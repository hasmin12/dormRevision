...
const Laundry = () => {
  ...
  const [selectedDay, setSelectedDay] = useState(new Date()); // Initialize selectedDay with a new Date object
  ...
  //     <Text style={{ color: disabled ? 'gray' : 'black' }}>{disabled ? `${label} (Full Schedule)` : label}</Text>
  // );
